...
echo -e "${BOLD}programs tests...${NORMAL}\n"

COUNT_PROGRAMS=`ls -1 ../programs/*.txt 2>/dev/null | wc -l`
if [ $COUNT_PROGRAMS -ne 0 ]; then
	for file in ../programs/*.txt; do
		...
		EXPECTED_PROGRAMS=expected_programs/${BASENAME}

		echo "${BOLD}- ${BASENAME}${NORMAL}"

		# Verifica se os arquivos são iguais
		diff -q ${PROG_PATH} ${EXPECTED_PROGRAMS}
	...
	echo -e "${VERMELHO}${BOLD}Não há arquivos compilados na pasta ${UNDERLINE}programs${NORMAL}\n"
    exit 1
fi
